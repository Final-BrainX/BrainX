...
`workspace-service`, `mcp-service`는 파일은 준비되어 있지만 아직 실제 apply 검증 전 기준으로 취급한다.
...
`gateway-secret.yaml`, `postgres-secret.yaml`, `workspace-secret.yaml`, `mcp-service-secret.yaml`, `admin-service-secret.yaml`, `grafana-secret.yaml` 같은 실제 Secret 파일이 Git에 추가되지 않았는지 확인한다.
...
- `kubectl -n brainx get svc user-service gateway-service admin-service workspace-service mcp-service`로 Prometheus active scrape 대상 Service 존재를 재확인
- `ingestion-service`, `commerce-service`, `intelligence-service`는 아직 Kubernetes Service가 없으므로 Prometheus scrape 대상에 넣지 않는지 확인
