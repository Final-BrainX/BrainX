...
- `/notes` 선택 버블의 `개요 노트 만들기`는 드래그한 텍스트를 새 노트 제목으로 삼아 AI 개요 문서를 만들고, 원본 선택 영역을 자동으로 `[[WikiLink]]`로 바꿉니다.
...
- The initial scrape targets are only the services already prepared in Kubernetes: `user-service`, `gateway-service`, and `admin-service`. `Discovery-Service` is intentionally excluded because its current `application.yml` exposes only `health` and `info`, not `prometheus`.
- `Workspace-Service`, `Ingestion-Service`, `Commerce-Service`, and `Intelligence-Service` already expose `/actuator/prometheus` and stamp `management.metrics.tags.application`, so the Prometheus config leaves commented target blocks ready for the moment those Services are added to Kubernetes.
