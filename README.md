...
- `/notes` 선택 버블의 `개요 노트 만들기`는 드래그한 텍스트를 새 노트 제목으로 삼아 AI 개요 문서를 만들고, 원본 선택 영역을 자동으로 `[[WikiLink]]`로 바꿉니다. 요청 직후 활성 노트가 바뀌면 원본 `noteId`와 현재 활성 노트를 비교해 해당 pending 요청을 실행하지 않고 정리합니다. mock notes 모드에서는 생성 노트와 원본 링크를 로컬 세션에만 반영하며 Workspace backend 저장을 호출하지 않습니다.
