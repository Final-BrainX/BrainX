...
- the Windows desktop package now reads branded icon assets from `brainx-electron/build/icon.png` and `brainx-electron/build/icon.ico`, so the executable, installer, and development BrowserWindow all share the same app icon instead of Electron defaults.
...
- vault assets can now be written into the local `assets/` directory, ZIP imports can expand directly into the active vault while recreating nested folders, and PDF/HTML/image imports now render from the local vault asset path instead of requiring the server asset endpoint.
