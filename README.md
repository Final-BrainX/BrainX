...
- Intelligence-Service의 비개발 profile은 `JWT_SECRET`·`SERVICE_TOKEN`을 각각 32 bytes 이상으로 강제하고, 기본 Swagger를 비활성화한다. Kafka producer는 broker ACK를 확인하며, consumer는 제한 재시도 후 SSOT의 `<topic>.dlq`로 보내고 DLQ publish 실패를 성공으로 처리하지 않는다.
...
- `/notes` 선택 버블의 `개요 노트 만들기`는 인라인 AI로 이동하지 않고 확인 모달을 먼저 표시합니다. 확인하면 드래그한 텍스트를 새 노트 제목으로 삼아 `개요` bullet과 `향후 작성할 내용` 체크리스트만 담은 짧은 AI 초안을 만들고, 원본 선택 영역을 실제 생성 제목의 `[[WikiLink]]`로 바꿉니다. 생성 후에는 원본 노트에 그대로 머물며 사용자가 링크를 눌렀을 때만 새 노트로 이동합니다. mock notes 모드에서는 생성 노트와 원본 링크를 로컬 세션에만 반영하며 Workspace backend 저장을 호출하지 않습니다.
...
- `/notes` 탭 `+`로 막 만든 노트는 서버 draft id 발급과 draft list 재조회 사이에도 in-flight local note로 보존해, 서버 동기화 타이밍 때문에 탭이 닫히거나 `새 노트1`, `새 노트2` 자동 넘버링이 되돌아가지 않게 합니다.
...
- `brainx_note_view_history_v1:{userId|guest}:{documentGroupId|local}`: `/notes`에서 마지막으로 조회한 노트 시각, `/graph` 시간 흐림 효과 기준
...
- `CLUSTERS`: 지식 클러스터 정의
- 전체 클러스터링은 기존 완료 snapshot을 1차 가드로 사용해 새 미분류 노트만 기존 클러스터 적합도 `0.75` 이상일 때 append하고, 나머지만 새 클러스터로 분류한다. 동일 클러스터의 첫 두 원본으로 만든 징검다리 노트는 해당 클러스터 소속을 상속한다.
- `seedNotes()`: 초기 노트 데이터
...
- Mcp-Service는 외부 agent 인증과 MCP `/mcp` transport를 소유합니다. 노트 검색/쓰기/삭제 tool은 Workspace/Intelligence public 또는 internal API를 통해 붙입니다.
...
- `brainx_delete_note`: Workspace 노트 삭제, `notes:write` 필요. `noteId`와 `mode=trash|permanent`를 모두 필수로 받으며 선택한 mode를 즉시 실행
...
Intelligence semantic search는 낮은 유사도 결과 노출을 줄이기 위해 `brainx.semantic-search.min-score` 기본값 `0.20`을 적용합니다. 운영에서는 `BRAINX_SEMANTIC_SEARCH_MIN_SCORE`로 `0.0`부터 `1.0` 사이에서 조정할 수 있습니다.
...
현재 관리자 화면은 실제 백엔드 데이터를 기준으로 사용자 플랜, 메모 수, 가입일, 최근 활동을 표시하며, 시간 표시는 모두 `Asia/Seoul` 기준으로 통일합니다. 사용자 목록의 플랜은 결제/환불 이력으로 추정하지 않고 Commerce-Service의 현재 구독 상태를 그대로 보여 주며, 상세 패널과 같은 값이 나오도록 맞췄습니다. 사용자 목록의 메모 수는 `Workspace-Service` note 원장 개수, 최근 활동은 실제 마지막 로그인 세션 시간으로 채웁니다. `최근 활동` 헤더는 현재 조회된 사용자 목록을 최신순(기본값) 또는 오래된순으로 전환하며, 활동 시각이 없는 사용자는 두 방향 모두 마지막에 유지합니다. 사용자 상세의 로그인 기기는 같은 기기/IP 접속을 하나로 합쳐 최신 접속 시간만 갱신하고, 최근 2건만 노출합니다. Electron 기반 데스크톱 앱 로그인은 관리자 화면에서 `BrainX App / Windows`, `BrainX App / macOS`처럼 앱 사용으로 구분해 표시합니다. 사용자 관리 화면에서는 정지된 계정을 바로 정지 취소할 수 있습니다.
...
- the Windows Electron main process also installs a fallback draggable region below the title-bar overlay whenever a remote renderer does not provide the bundled `brainx-next` drag region, while reserving the native window-control area on the right.
