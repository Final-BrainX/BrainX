...
개발자용 fallback으로 API key도 유지합니다. 사용자 JWT로 `POST /api/v1/mcp/api-clients`에 `{ "name": "...", "scopes": ["whoami", "notes:read", "ai:search", "notes:write"] }`를 보내 `apiKeyOnce`를 발급하고, `Authorization: Bearer bxk_live_...` 또는 `X-BrainX-Api-Key`로 사용할 수 있습니다. `/settings`의 MCP API Keys 패널도 같은 네 scope를 기본으로 보냅니다.
