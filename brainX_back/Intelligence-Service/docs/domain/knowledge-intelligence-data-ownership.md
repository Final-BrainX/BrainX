...
다른 BrainX 서비스와의 구체적인 계약, 이벤트, port 경계는 `docs/technical/cross-service-integration-map.md`에서 확인한다.
...
- Agent 실험 thread, message, 승인 대기/실행 action
- 작성 보조, 링크 추천, 브릿지 후보, 폴더 정리 제안 이벤트
- token usage와 비용 추정 요청

Agent action은 사용자의 승인 전까지 실행 가능한 제안이며, 승인 뒤에도 원본 Workspace 상태는 Workspace API로 변경한다. Intelligence-Service는 action proposal과 승인/거절/실행 결과를 저장하지만, 생성되거나 수정된 노트 원본을 직접 소유하지 않는다.
...
| Agent 실험 경험 | `intelligence_agent_threads`, `intelligence_agent_messages`, `intelligence_agent_actions` |
