...
- 내부 전체 분석 범위는 `userId + documentGroupId` 안의 indexed/searchable note다.
...
- public `/api/v1/ai/link-suggestions`는 `connection` feature에서 source-only `LLM_ONLY` 분석 경로를 사용한다. 이 public 경로는 embedding index 없이 active markdown projection만 요구한다. public API의 자세한 계약과 제한은 `docs/technical/connection-api.md`를 기준으로 확인한다.
...
- 전체 자동 연결과 비교 CLI에서는 indexed/searchable note set으로 실행한다. public 연결 추천은 같은 검증/필터를 쓰되 graph AI source-ready note set에서 요청 source note 하나만 source로 삼아 최대 1회 호출한다.
