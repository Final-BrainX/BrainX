...
- `ExplorationService.generateNoteSummary(...)`: `/graph` hover tooltip과 `/notes` 컨텍스트 패널의 세줄 요약 생성을 `note-summary-chat` featureId로 기록한다. 모델은 사용자 기본 채팅 모델이 아니라 `brainx.note-summary.model` / `BRAINX_NOTE_SUMMARY_MODEL`이며 기본값은 `gpt-5.4-nano`다.
