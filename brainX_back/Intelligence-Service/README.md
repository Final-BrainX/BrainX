...
`/api/v1/ai/link-suggestions`는 public 연결 추천에서 `LLM_ONLY` 전략을 사용하고, 응답에 source markdown anchor text와 offset을 포함한다. 클라이언트는 추천 수락 시 source note 본문을 `[[...]]` wiki link로 저장해 Workspace 링크 ledger와 graph projection이 기존 content save 흐름으로 동기화되게 한다.
