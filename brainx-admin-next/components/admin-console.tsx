...
const titles: Record<ConsoleView, { title: string; desc: string }> = {
  admins: {
    title: "관리자 관리",
    desc: "관리자 계정 추가와 권한 관리를 할 수 있어요",
  },
  dashboard: {
    title: "모니터링",
    desc: "BrainX 운영 지표를 실시간으로 확인해요",
  },
  users: {
    title: "사용자 관리",
    desc: "BrainX 사용자 목록과 플랜 상태를 관리해요",
  },
  support: {
    title: "문의 응답",
    desc: "고객 문의를 확인하고 담당자에게 배정해요",
  },
  billing: {
    title: "결제 관리",
    desc: "결제 내역, 구독, 환불 및 요금제를 관리해요",
  },
};
    ...
    <svg viewBox="0 0 ${width} ${height}" width="100%" height="100%" role="img" aria-label="${escapeHtml(title)}">
      ...
      ${labels
        .map((label, index) => {
          ...
          const x =
            padding.left +
            (innerWidth / Math.max(labels.length - 1, 1)) * index;
    ...
    <svg viewBox="0 0 ${width} ${height}" width="100%" height="100%" role="img" aria-label="${escapeHtml(title)}">
      ...
      ${values
        .map((value, index) => {
          const x =
            padding.left +
            (innerWidth / Math.max(values.length, 1)) * index +
            4;
      ...
      ${labels
        .map((label, index) => {
          ...
          const x =
            padding.left +
            (innerWidth / Math.max(labels.length, 1)) * index +
            innerWidth / Math.max(values.length, 1) / 2;
          return `<text x="${x}" y="${height - 12}" text-anchor="middle" fill="#6b7280" font-size="11">${escapeHtml(label)}</text>`;
        })
        .join("")}
...
type KafkaLagSnapshotLike = Pick<
...
function kafkaLagTone(view: KafkaLagView | null | undefined): {
  background: string;
  color: string;
  dot?: string;
} {
  if (!view || view.kafkaLagState == null)
    return { background: "#f5f5f4", color: "#57534e" };
  if (view.kafkaLagState === "BROKER_UNREACHABLE")
    return { background: "#fef2f2", color: "#dc2626", dot: "#ef4444" };
  if (
    view.kafkaLagState === "NO_COMMITTED_OFFSETS" ||
    view.kafkaLagState === "CONFIG_MISSING"
  ) {
    return { background: "#f5f5f4", color: "#57534e" };
  }
...
function kafkaLagDetail(view: KafkaLagView | null | undefined) {
  ...
  if (view.kafkaLagState === "NO_COMMITTED_OFFSETS")
    return "커밋된 offset이 없어 미집계 상태";
  if (view.kafkaLagState === "CONFIG_MISSING")
    return "consumer group id 설정이 필요합니다";
  if (view.kafkaLagMessages == null) return "lag를 읽지 못했습니다";
  return view.kafkaLagDetail ?? "최신 consumer group lag";
}

function kafkaLagDisplay(
  view: KafkaLagView | KafkaLagSnapshotLike | null | undefined,
): KafkaLagView | null {
  if (!view) return null;
  return {
    consumerGroupId:
      "consumerGroupId" in view
        ? (view.consumerGroupId ?? null)
        : (view.kafkaConsumerGroupId ?? null),
    kafkaLagState: view.kafkaLagState,
    kafkaLagMessages: view.kafkaLagMessages,
    kafkaLagDetail: view.kafkaLagDetail ?? null,
    warningThreshold:
      "warningThreshold" in view ? view.warningThreshold : undefined,
    criticalThreshold:
      "criticalThreshold" in view ? view.criticalThreshold : undefined,
  };
}
...
function Tag({
  ...
  meta: { background: string; color: string; dot?: string };
...
function downloadAdminReport(payload: {
  ...
  const endDate = payload.lastUpdatedAt
    ? new Date(payload.lastUpdatedAt)
    : generatedAt;
  const trendLength = Math.max(
    payload.activeUserSeries.length,
    payload.revenueBars.length,
    14,
  );
  const trendLabels = buildTrendLabels(trendLength, endDate);
  const activeTrend = normalizeSeries(
    payload.activeUserSeries,
    normalizeSeries(payload.activeUserTrendMeta.values, [0]),
  );
  const revenueTrend = normalizeSeries(
    payload.revenueBars,
    normalizeSeries(payload.revenueTrendMeta.values, [0]),
  );
  const desktopDownloadTrend = normalizeSeries(
    payload.desktopDownloadSeries,
    normalizeSeries(payload.desktopDownloadTrendMeta.values, [0]),
  );
  const trendDates = trendLabels.slice(
    -Math.max(
      activeTrend.length,
      revenueTrend.length,
      desktopDownloadTrend.length,
    ),
  );
  ...
  <body>
    ...
    <div class="split">
      <div class="chart-card">
        ...
        ${buildLineChartSvg(
          trendDates,
          activeTrendRows.map((row) => row.activeUsers),
          "최근 14일 활성 사용자 추이",
        )}
      </div>
      <div class="chart-card">
        ...
        ${buildBarChartSvg(
          trendDates,
          activeTrendRows.map((row) => row.revenue),
          "최근 14일 매출 추이",
        )}
      </div>
      <div class="chart-card">
        ...
        ${buildBarChartSvg(
          trendDates,
          activeTrendRows.map((row) => row.desktopDownloads),
...
export function AdminConsole() {
  ...
  const [billingTab, setBillingTab] = useState<
    "history" | "subscriptions" | "failed" | "plans"
  >("history");
    ...
    adminApi
      ...
      .catch((error) => {
        ...
        if (
          ...
          getSession()?.accessToken === sessionToken
        ) {
          clearSession();
          router.replace("/login");
          return;
        ...
        setAuthReady(true);
        setAdminError(
          errorMessage || "관리자 세션 확인 중 오류가 발생했습니다.",
        );
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    setProfileImage(
      window.localStorage.getItem(ADMIN_PROFILE_IMAGE_STORAGE_KEY),
    );
  }, []);
  ...
  const reloadAdminData = async () => {
    ...
    try {
      ...
      setSelectedInquiry((current) =>
        data.inquiries.some((item) => item.id === current)
          ? current
          : (data.inquiries[0]?.id ?? ""),
      );
    } catch (error) {
      const sessionAdmin =
        getSession()?.admin ?? fallbackAdminBootstrap.adminProfile;
  ...
  useEffect(() => {
    if (
      screen === "admins" &&
      adminData &&
      adminData.adminProfile.role !== "owner"
    ) {
      setScreen("dashboard");
      return;
    }
  ...
  const activeInquiry =
    adminData?.inquiries.find((item) => item.id === selectedInquiry) ??
    adminData?.inquiries[0];
  const failedPayments =
    adminData?.billingTransactions.filter(
  ...
  const syncAdminProfile = (profile: AdminBootstrap["adminProfile"]) => {
    ...
    setAdminData((current) =>
      current ? { ...current, adminProfile: profile } : current,
    );
  };
        ...
        <main className="main" style={{ width: "100%" }}>
          <header className="topbar">
            <div style={{ minWidth: 0 }}>
              ...
              <p
                style={{ margin: "2px 0 0", color: "#a8a29e", fontSize: 12.5 }}
              >
                실제 관리자 데이터를 불러오는 중입니다
              </p>
            </div>
          </header>
  ...
  return (
    <div className="admin-shell">
      <AdminSidebar
        ...
        supportBadge={
          adminData.inquiries.filter((item) => item.status === "pending").length
        }
      ...
      <main className="main">
        <header className="topbar">
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ margin: "2px 0 0", color: "#a8a29e", fontSize: 12.5 }}>
              {titles[screen].desc}
            </p>
          </div>
          <div style={{ flex: 1 }} />
          ...
          <button
            className="btn"
            aria-label="알림"
            onClick={() => notify("읽지 않은 알림 3건이 있습니다")}
          ...
          <div className="mono" style={{ color: "#57534e", fontSize: 13 }}>
            {now}
            <span style={{ color: "#a8a29e", fontSize: 11 }}> KST</span>
          </div>
        </header>
        ...
        <section className="content">
          ...
          {screen === "admins" ? (
            <AdminAccountsPanel
              currentAdmin={adminData.adminProfile}
              onCurrentAdminUpdated={syncAdminProfile}
              onToast={notify}
            />
          ) : null}
        </section>
  ...
  return (
    <div className={`data-state ${error ? "error" : ""}`}>
      <span
        className="dot"
        style={{
          background: error ? "#ef4444" : "#64748b",
          animation: loading ? "pulse 1.5s infinite" : undefined,
        }}
      />
      <span>
        {error
          ? "관리자 API 연결 실패 - 로컬 fallback 데이터를 표시 중입니다"
          : "관리자 API 데이터를 불러오는 중입니다"}
      </span>
      {error ? (
...
function Dashboard({
  ...
  const desktopDownloadUsers = normalizeNumber(
    overviewSummary.desktopDownloadUsers,
    0,
  );
  const desktopDownloadCount = normalizeNumber(
    overviewSummary.desktopDownloadCount,
    0,
  );
  ...
  const activeUserSeries = normalizeSeries(
    (data as AdminBootstrap & { traffic?: number[] }).activeUserSeries ??
      (data as AdminBootstrap & { traffic?: number[] }).traffic,
    normalizeSeries(activeUserTrendMeta.values, [0]),
  );
  const revenueBars = normalizeSeries(
    data.revenueBars,
    normalizeSeries(revenueTrendMeta.values, [0]),
  );
  const desktopDownloadSeries = normalizeSeries(
    data.desktopDownloadSeries,
    normalizeSeries(desktopDownloadTrendMeta.values, [0]),
  );
  ...
  const [openModal, setOpenModal] = useState<
    "snapshots" | "health" | "logs" | null
  >(null);
  const HISTORY_PREVIEW_COUNT = 5;
  const intelligenceService =
    data.services.find((service) => service.name === "Intelligence-Service") ??
    data.services.find((service) => service.name === "AI-Service") ??
    null;
  ...
  const deleteHealthSnapshot = async (id: string) => {
    ...
    setHealthSnapshots((prev) =>
      prev.filter((item) => item.healthSnapshotId !== id),
    );
  ...
  const orderedHealthSnapshots = healthSnapshots
    .slice()
    .sort(
      (left, right) =>
        new Date(left.capturedAt).getTime() -
  ...
  const latestMonitoringSnapshot =
    ...
    null;
  const latestMonitoringUpdatedAt =
    latestMonitoringSnapshot?.capturedAt ??
    data.monitoringSnapshots[data.monitoringSnapshots.length - 1]?.capturedAt ??
    null;
  ...
  const activeTrendLabels = buildTrendLabels(
    activeUserSeries.length,
    latestMonitoringUpdatedAt,
  );
  const revenueTrendLabels = buildTrendLabels(
    revenueBars.length,
    latestMonitoringUpdatedAt,
  );
  const desktopDownloadTrendLabels = buildTrendLabels(
    desktopDownloadSeries.length,
    latestMonitoringUpdatedAt,
  );
  ...
  const renderSnapshotRow = (snapshot: AdminMonitoringSnapshot) => (
    ...
    >
      <span
        className="mono"
        style={{ width: 130, color: "#78716c", fontSize: 11 }}
      >
        {formatHistoryTime(snapshot.capturedAt)}
      </span>
      <span className="mono" style={{ flex: 1, fontSize: 12 }}>
        매출 {money(snapshot.monthlyRevenue)} · 구독{" "}
        {snapshot.activeSubscriptions} · MRR {money(snapshot.mrr)} · 활성{" "}
        {snapshot.activeUsers} · 다운로드{" "}
        {normalizeNumber(snapshot.desktopDownloadUsers, 0)}/
        {normalizeNumber(snapshot.desktopDownloadCount, 0)} · Kafka{" "}
        {kafkaLagLabel(kafkaLagDisplay(snapshot))}{" "}
        {snapshot.kafkaLagMessages == null
          ? ""
          : `(${snapshot.kafkaLagMessages.toLocaleString("ko-KR")} msgs)`}
      </span>
      {snapshot.persisted ? (
        <button
          className="btn danger"
          title="삭제"
          onClick={() => deleteSnapshot(snapshot.snapshotId)}
          style={{
            width: 30,
            height: 30,
            padding: 0,
  ...
  const renderHealthRow = (snapshot: AdminServiceHealthSnapshot) => (
    ...
    >
      <span
        className="dot"
        style={{ background: healthMeta(snapshot.state).dot }}
      />
      <span
        className="mono"
        style={{ width: 130, color: "#78716c", fontSize: 11 }}
      ...
      <span className="mono" style={{ flex: 1, fontSize: 12 }}>
        {snapshot.serviceName} · {snapshot.latencyMs}ms
      </span>
      <button
        className="btn danger"
        title="삭제"
        onClick={() => deleteHealthSnapshot(snapshot.healthSnapshotId)}
  ...
  const renderLogRow = (log: AdminBootstrap["logs"][number], index: number) => (
    <div
      key={`${log.service}-${log.time}-${index}-${log.message}`}
      style={{
        display: "flex",
        alignItems: "center",
        gap: 14,
    ...
    >
      ...
      <span
        className="mono"
        style={{ width: 150, color: "#57534e", fontSize: 12 }}
      >
        {log.service}
      </span>
      <span style={{ flex: 1, color: "#44403c", fontSize: 13 }}>
        {log.message}
      </span>
      <span className="mono" style={{ color: "#a8a29e", fontSize: 11.5 }}>
        {log.time}
      </span>
    </div>
  );
    ...
    <>
      ...
      >
        ...
        >
          ...
          <span>
            실시간 · 마지막 업데이트{" "}
            {latestMonitoringUpdatedAt
              ? formatKstDateTime(latestMonitoringUpdatedAt)
              : "-"}
          </span>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn" onClick={() => void handleRefresh()}>
            <RefreshCw size={15} />
            새로고침
          </button>
      ...
      >
        {visibleKpis.map((item) => (
          <div className="card" key={item.label}>
            ...
            <div style={{ marginTop: 4, color: "#a8a29e", fontSize: 12 }}>
              {item.sub}
            </div>
          </div>
        ))}
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              ...
              <div style={{ marginTop: 2, color: "#a8a29e", fontSize: 12 }}>
                현재 활성 사용자{" "}
                <b className="mono" style={{ color: "#44403c" }}>
                  {overviewSummary.activeUsers.toLocaleString("ko-KR")}
                </b>{" "}
                · {activeUserTrendMeta.periodLabel} ·{" "}
                {activeUserTrendMeta.source}
              </div>
            </div>
        ...
        <div className="card">
          ...
          {data.services.map((service) => (
            ...
            >
              <span
                className="dot"
                style={{ background: healthMeta(service.state).dot }}
              />
              <span
                className="mono"
              ...
              <span
                className="mono"
                style={{
                  color: healthMeta(service.state).text,
                  fontSize: 12,
                  fontWeight: 600,
                }}
              >
      ...
      <div className="grid-bottom-top">
        <div className="card revenue-card monitoring-chart-card">
          <div
            className="monitoring-card-header"
            style={{ display: "flex", justifyContent: "space-between" }}
          >
            <div className="card-title">
              매출 분석{" "}
              <span style={{ color: "#a8a29e", fontSize: 12, fontWeight: 500 }}>
                {revenueTrendMeta.periodLabel}
              </span>
            </div>
            <span
              className="mono"
              style={{ color: "#0d9488", fontSize: 13, fontWeight: 600 }}
            >
              {revenueTrendMeta.timezone}
            </span>
          </div>
            ...
            <div className="revenue-chart revenue-chart-compact">
              {revenueBars.map((bar, index) => (
                <div
                  ...
                  style={{
                    flex: 1,
                    height: `${Math.max(4, (bar / maxRevenueBar) * 100)}%`,
                    borderRadius: "7px 7px 3px 3px",
                    background: index > 10 ? "#0d9488" : "#d6d3d1",
                  }}
                />
              ))}
            </div>
        ...
        <div className="grid-bottom-side">
          <div className="card monitoring-chart-card">
            ...
            >
              ...
              >
                {desktopDownloadTrendMeta.timezone}
              </span>
            </div>
            <div
              style={{
            ...
            >
              <div className="mono" style={{ fontSize: 24, fontWeight: 600 }}>
                {desktopDownloadCount.toLocaleString("ko-KR")}
              </div>
              <div style={{ color: "#6b7280", fontSize: 12 }}>
                사용자 {desktopDownloadUsers.toLocaleString("ko-KR")}명
              </div>
            </div>
            <div className="monitoring-chart-body monitoring-chart-body-compact">
              <div
                className="revenue-chart monitoring-mini-chart"
                style={{ height: 120 }}
              >
                {desktopDownloadSeries.map((bar, index) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "7px 7px 3px 3px",
                      background:
                        index > Math.max(0, desktopDownloadSeries.length - 4)
                          ? "#2563eb"
                          : "#bfdbfe",
                    }}
                  />
              ...
              <div className="monitoring-footnote">
                {desktopDownloadTrendMeta.periodLabel} ·{" "}
                {desktopDownloadTrendMeta.source}
              </div>
            </div>
          </div>
          <AlertMetric
      ...
      <div className="grid-bottom-metrics">
        ...
        <div className="card monitoring-metric-card">
          ...
          >
            {latestKafkaLagView?.kafkaLagMessages == null
              ? "-"
              : latestKafkaLagView.kafkaLagMessages.toLocaleString(
                  "ko-KR",
                )}{" "}
            <span style={{ color: "#a8a29e", fontSize: 12 }}>msgs</span>
          </div>
          <div style={{ marginTop: 2, color: "#a8a29e", fontSize: 11 }}>
            {latestKafkaLagView?.consumerGroupId ?? "intelligence-service"} ·{" "}
            {kafkaLagDetail(latestKafkaLagView)}
          </div>
          <div style={{ marginTop: 4, color: "#a8a29e", fontSize: 11 }}>
            경고 기준{" "}
            {kafkaLagThresholds(
              latestKafkaLagView,
            ).warningThreshold.toLocaleString("ko-KR")}{" "}
            msgs · 심각 기준{" "}
            {kafkaLagThresholds(
              latestKafkaLagView,
            ).criticalThreshold.toLocaleString("ko-KR")}{" "}
            msgs
          </div>
        </div>
      </div>
      <div className="table-wrap">
        ...
        >
          ...
          <button className="btn" onClick={() => setOpenModal("logs")}>
            전체 보기
          </button>
        </div>
      ...
      <div className="grid-wide" style={{ marginTop: 20 }}>
        <div className="card">
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              {snapshots.length > HISTORY_PREVIEW_COUNT ? (
                <button
                  className="btn"
                  onClick={() => setOpenModal("snapshots")}
                >
                  전체 보기
                </button>
              ) : null}
          ...
          {snapshots.length === 0 && (
            <div style={{ color: "#a8a29e", fontSize: 12, padding: "8px 0" }}>
              저장된 기록이 없어요
            </div>
          )}
          {reversedSnapshots
            .slice(0, HISTORY_PREVIEW_COUNT)
            .map(renderSnapshotRow)}
        </div>
        <div className="card">
          <div
            style={{
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ color: "#a8a29e", fontSize: 11 }}>
                {healthSnapshots.length}건
              </span>
              {healthSnapshots.length > HISTORY_PREVIEW_COUNT ? (
                <button className="btn" onClick={() => setOpenModal("health")}>
                  전체 보기
                </button>
              ) : null}
            </div>
      ...
      {openModal === "snapshots" ? (
        <ListModal
          title={`모니터링 기록 (${snapshots.length}건)`}
        ...
        >
          {snapshots.length === 0 ? (
            ...
            reversedSnapshots.map(renderSnapshotRow)
          )}
        </ListModal>
      ) : null}
      {openModal === "health" ? (
        <ListModal
          title={`서비스 체크 기록 (${healthSnapshots.length}건)`}
          onClose={() => setOpenModal(null)}
        >
          {healthSnapshots.length === 0 ? (
            <div style={{ color: "#a8a29e", fontSize: 12, padding: "8px 0" }}>
            ...
            reversedHealthSnapshots.map(renderHealthRow)
          )}
        </ListModal>
      ) : null}
      {openModal === "logs" ? (
        <ListModal
          title={`실시간 에러 로그 & 알림 (${data.logs.length}건)`}
          onClose={() => setOpenModal(null)}
        >
...
function AdminProfileRail({
  ...
  const [accounts, setAccounts] = useState<
    Awaited<ReturnType<typeof adminApi.listAdminAccounts>>
  >([]);
  ...
  const [editingField, setEditingField] = useState<
    "loginId" | "email" | "password" | null
  >(null);
  const railRef = useRef<HTMLElement | null>(null);
  const avatarInputRef = useRef<HTMLInputElement | null>(null);
  const currentAccount =
    accounts.find((account) => account.adminId === admin.adminUserId) ??
    accounts[0];
  const messageViewer: AdminMessageViewer = {
    adminUserId: admin.adminUserId,
    name: admin.name,
  };
  const unreadMessages = messages.filter((message) => !message.isRead);
  const availableRecipients = accounts.filter(
    (account) => account.adminId !== admin.adminUserId,
  );
      ...
      await adminApi.sendAdminMessage(
        {
          ...
          recipientAdminUserIds:
            messageScope === "SELECTED" ? selectedRecipientIds : [],
          body: nextBody,
        },
        messageViewer,
      );
  ...
  const openMessageModal = async () => {
    ...
    await Promise.all(
      unreadIds.map((messageId) =>
        adminApi
          .markAdminMessageRead(messageId, messageViewer)
          .catch(() => null),
      ),
    );
    await loadMessages();
  ...
  const saveLoginId = async () => {
    ...
    try {
      const { admin: updated } = await adminApi.updateAdminAccount(
        currentAccount.adminId,
        { loginId: nextLoginId },
      );
      setAccounts((prev) =>
        prev.map((account) =>
          account.adminId === updated.adminId ? updated : account,
        ),
      );
      onToast("아이디를 변경했어요");
      setEditingField(null);
      ...
      onToast(
        error instanceof Error ? error.message : "아이디 변경에 실패했어요",
      );
    }
  };
  ...
  const saveEmail = async () => {
    try {
      ...
      onToast(
        error instanceof Error ? error.message : "이메일 변경에 실패했어요",
      );
    }
  };
    ...
    <aside className="dashboard-rail">
      <section className="rail-card admin-setting-card" ref={railRef}>
        <input
          ...
          onChange={(event) =>
            updateAvatarImage(event.target.files?.[0] ?? null)
          }
        />
        <div className="admin-setting-hero">
          <button
            className="admin-setting-avatar-button"
            type="button"
          ...
          >
            {profileImage ? (
              <img
                src={profileImage}
                alt=""
                className="admin-setting-avatar-image"
              />
            ) : (
              <div className="admin-setting-avatar">
          ...
          <span className="admin-setting-role">
        ...
        <div className="admin-setting-meta">
          <div className="admin-setting-meta-row">
            ...
            <span className="admin-setting-meta-value mono">
              {currentAccount?.loginId ?? "-"}
            </span>
            <button
              className="admin-setting-icon-button"
              type="button"
              onClick={() => setEditingField("loginId")}
              aria-label="아이디 수정"
              title="아이디 수정"
            >
              <Pencil size={12} />
          ...
          <div className="admin-setting-meta-row">
            ...
            <span className="admin-setting-meta-value">
              {admin.email ?? "-"}
            </span>
            <button
              className="admin-setting-icon-button"
          ...
          <div className="admin-setting-meta-row">
            ...
            <span className="admin-setting-meta-value">
              {unreadMessages.length}건
            </span>
            <button
              className={`admin-setting-link${unreadMessages.length === 0 ? " muted" : ""}`}
              type="button"
              onClick={() => void openMessageModal()}
            >
              읽음
            </button>
          </div>
      ...
      <section className="rail-card rail-admin-list">
        <div className="rail-head">
          <div>
            ...
            <div className="rail-subtitle">
              등록된 관리자 계정을 빠르게 확인해요
            </div>
          </div>
          <span className="mono" style={{ color: "#6b7280", fontSize: 12 }}>
            {loadingAccounts ? "불러오는 중" : `${accounts.length}명`}
          </span>
        </div>
        <div className="rail-admin-list-body">
          {accounts.map((account) => (
            ...
            >
              <div className="rail-mini-avatar">
                {account.adminId === admin.adminUserId && profileImage ? (
                  <img
                    src={profileImage}
                    alt=""
                    className="rail-mini-avatar-image"
                  />
                ) : (
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <div
                  style={{ fontSize: 13.5, fontWeight: 700, color: "#111827" }}
                >
                  {account.name}
                </div>
              ...
              <Tag meta={roleBadgeStyle[account.role] ?? roleBadgeStyle.admin}>
          ...
          {!loadingAccounts && accounts.length === 0 ? (
            <div
              style={{ color: "#6b7280", fontSize: 13, padding: "12px 2px" }}
            >
              관리자 계정이 없어요
            </div>
          ) : null}
      ...
      <section className="rail-card admin-message-card">
        ...
        <AdminMessageFeed
          ...
          resolveRecipientLabel={(message) =>
            describeAdminMessageAudience(message, accounts)
          }
        />
...
function describeAdminMessageAudience(
  message: AdminMessage,
  accounts: Array<{ adminId: string; name: string }>,
) {
  if (message.recipientScope === "ALL") return "전체";
  const names = message.recipientAdminUserIds
    .map(
      (adminId) =>
        accounts.find((account) => account.adminId === adminId)?.name,
    )
...
function AdminMessageFeed({
  ...
  if (loading) {
    return (
      <div className="admin-message-empty">메시지를 불러오는 중이에요</div>
    );
  }

  if (messages.length === 0) {
    return (
      <div className="admin-message-empty">아직 주고받은 메시지가 없어요</div>
    );
  }
      ...
      {messages.map((message) => {
        ...
        return (
          <div
            ...
            className={`admin-message-row${mine ? " mine" : ""}`}
          >
            <div className="admin-message-meta">
              <span>{mine ? "나" : message.senderName}</span>
              <span>{resolveRecipientLabel(message)}</span>
              {!mine && !message.isRead ? (
                <span className="admin-message-unread">NEW</span>
              ) : null}
            </div>
            <div className={`admin-message-bubble${mine ? " mine" : ""}`}>
              {message.body}
            </div>
            <div className="admin-message-time">
              {formatHistoryTime(message.sentAt)}
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className={`admin-message-composer${compact ? " compact" : ""}`}>
      <div className="admin-message-audience">
        <button
          className={`admin-message-scope${scope === "ALL" ? " active" : ""}`}
          type="button"
        ...
        <button
          className={`admin-message-scope${scope === "SELECTED" ? " active" : ""}`}
          type="button"
      ...
      {scope === "SELECTED" ? (
        <div className="admin-message-recipient-list">
          ...
          {recipientOptions.length === 0 ? (
            <div className="admin-message-empty inline">
              보낼 다른 관리자가 없어요
            </div>
          ) : null}
        </div>
...
function AdminMessageModal({
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className="list-modal admin-message-modal"
...
function AlertMetric({
  ...
  const bar =
    tone === "good"
      ? { width: "82%", background: "linear-gradient(90deg,#14b8a6,#0f766e)" }
      : tone === "warn"
        ? { width: "76%", background: "linear-gradient(90deg,#f59e0b,#d97706)" }
        : {
            width: "62%",
            background: "linear-gradient(90deg,#94a3b8,#64748b)",
          };
  const valueColor =
    tone === "good" ? "#0f766e" : tone === "warn" ? "#d97706" : "#64748b";
  return (
    <div className={`card${className ? ` ${className}` : ""}`}>
      ...
      <div style={{ display: "flex", gap: 20 }}>
        {[
          { label: left, value: leftValue },
          { label: right, value: rightValue },
        ].map((item) => (
          <div key={item.label} style={{ flex: 1 }}>
            <div style={{ color: "#a8a29e", fontSize: 11 }}>{item.label}</div>
            <div
              className="mono"
              style={{ color: valueColor, fontSize: 22, fontWeight: 600 }}
            >
              {item.value}
            </div>
          </div>
      ...
      {detail ? (
        <div style={{ marginTop: 8, color: "#a8a29e", fontSize: 11 }}>
          {detail}
        </div>
      ) : null}
      <div
  ...
  const toggleAllVisible = () => {
    setSelectedIds((current) => {
      if (allVisibleSelected)
        return current.filter((id) => !rows.some((user) => user.id === id));
    ...
    <>
      <div className="toolbar">
        ...
        <select
          className="select"
          value={plan}
          onChange={(event) => setPlan(event.target.value as Plan | "all")}
        >
        ...
        <span style={{ color: "#78716c", fontSize: 13 }}>
          결과 <b style={{ color: "#1c1917" }}>{rows.length}</b>명
        </span>
      </div>
      {selectedIds.length > 0 ? (
        <div className="bulkbar">
          ...
          <button
            className="btn"
            onClick={() => ensureSelection() && setBulkPlanModalOpen(true)}
          >
            <Repeat2 size={15} />
            플랜 변경
          </button>
          <button
            className="btn"
            onClick={() => ensureSelection() && setBulkSuspendModalOpen(true)}
          >
            <Ban size={15} />
            계정 정지
          </button>
          <button
            className="btn danger"
            onClick={() => ensureSelection() && setBulkWithdrawModalOpen(true)}
          >
            <X size={15} />
            탈퇴 처리
          </button>
          <button
            className="btn"
            onClick={() => ensureSelection() && setBulkNoticeModalOpen(true)}
          >
            <Megaphone size={15} />
            공지 발송
          </button>
          ...
          <tbody>
            {rows.map((user) => (
              <tr
                ...
                onClick={() => setDetailUser(user as AdminUserDetail)}
              >
                ...
                <td>
                  <Tag meta={planStyle[user.plan]}>{planLabel[user.plan]}</Tag>
                </td>
                <td>
                  <Tag meta={userStatus[user.status]}>
      ...
      {bulkPlanModalOpen ? (
        <BulkPlanChangeModal
          ...
          onApply={async (targetPlanId) => {
            await adminApi.runUserBulkAction(selectedIds, "CHANGE_PLAN", {
              targetPlanId,
            });
            await onReload();
            onToast(
              `${selectedIds.length}명의 플랜을 ${planLabel[targetPlanId]}로 변경했어요`,
            );
      ...
      {bulkSuspendModalOpen ? (
        <SuspendUsersModal
          ...
          onApply={async ({ reason, suspendedDays }) => {
            await adminApi.runUserBulkAction(selectedIds, "SUSPEND", {
              reason,
              suspendedDays,
            });
            await onReload();
            onToast(
              `${selectedIds.length}명의 계정을 ${suspendedDays}일 정지했어요`,
            );
      ...
      {bulkWithdrawModalOpen ? (
        <WithdrawUsersModal
          ...
          onApply={async (reason) => {
            await adminApi.runUserBulkAction(selectedIds, "WITHDRAW", {
              reason,
            });
            await onReload();
...
function Avatar({ user, size = 38 }: { user: AdminUser; size?: number }) {
  ...
  const color =
    palette[
      Math.abs(
        user.name.charCodeAt(0) + user.name.charCodeAt(user.name.length - 1),
      ) % palette.length
    ];
  return (
    <span
...
function UserDetailDrawer({
  ...
  const suspendButtonLabel =
    detail.status === "suspended" ? "정지 취소" : "계정 정지";
  const suspendButtonIcon =
    detail.status === "suspended" ? <RefreshCw size={15} /> : <Ban size={15} />;
      ...
      >
        ...
        <div className="drawer-body">
          ...
          <div className="drawer-status">
            <Tag meta={userStatus[detail.status]}>
              {userStatus[detail.status].label}
            </Tag>
          </div>
          <div className="detail-grid">
            <DetailStat
              label="메모 수"
              value={detail.notes.toLocaleString("ko-KR")}
            />
            <DetailStat
              label="현재 플랜"
              value={planLabel[detail.plan]}
              tag={detail.plan}
            />
          ...
          <section className="device-section">
            ...
            <div className="device-list">
              {sessions.length === 0 ? (
                <p style={{ color: "#a8a29e", fontSize: 13, padding: "8px 0" }}>
                  로그인 기록이 없습니다
                </p>
              ) : (
                sessions.map((session) => (
                  <div
                    className={`device-item ${session.current ? "current" : ""}`}
                    key={session.id}
                  >
          ...
          <section className="activity-section">
            ...
            {detail.activities.map((activity, index) => (
              <div
                ...
                key={`${activity.text}-${activity.time}-${index}`}
              >
      ...
      {planModalOpen ? (
        <PlanChangeModal
          ...
          onApply={async (plan) => {
            ...
            await onReload();
            onToast(
              `${detail.name}의 플랜을 ${planLabel[plan]}으로 변경했어요`,
            );
...
function DetailStat({
  ...
  return (
    <div className="detail-stat">
      <span>{label}</span>
      {tag ? (
        <Tag meta={planStyle[tag]}>{value}</Tag>
      ) : (
  ...
  return (
    <div className="modal-backdrop">
      <div className="price-modal plan-change-modal">
        ...
        <label className="reply-label" htmlFor="suspend-reason">
          정지 사유
        </label>
        <textarea
          id="suspend-reason"
        ...
        />
        <label className="reply-label" htmlFor="suspend-days">
          정지 일수
        </label>
        <input
          id="suspend-days"
          className="search user-search"
          value={suspendedDays}
          onChange={(event) => setSuspendedDays(event.target.value)}
          placeholder="예: 7"
        />
        <div className="modal-actions">
          <button onClick={onClose}>취소</button>
          <button
            onClick={() =>
              onApply({
                reason: reason.trim(),
                suspendedDays: Math.max(1, Number(suspendedDays) || 1),
              })
            }
          >
            정지 적용
  ...
  return (
    <div className="modal-backdrop">
      <div className="price-modal plan-change-modal">
        ...
        <h2>탈퇴 처리 확인</h2>
        <p>
          {count}명의 탈퇴 처리를 요청합니다. 이 작업은 되돌리기 어렵습니다.
        </p>
        <label className="reply-label" htmlFor="withdraw-reason">
          탈퇴 사유
        </label>
        <textarea
          id="withdraw-reason"
          ...
          value={reason}
          onChange={(event) => setReason(event.target.value)}
          placeholder="내부 관리용 사유를 남길 수 있습니다."
        />
        <div className="modal-actions">
          <button onClick={onClose}>취소</button>
    ...
    <div className="modal-backdrop">
      ...
      >
        ...
        <div style={stackStyle}>
          <div>
            <label className="reply-label" htmlFor="notice-type">
              공지 유형
            </label>
            <select
              id="notice-type"
          ...
          <div>
            <label className="reply-label" htmlFor="notice-title">
              공지 제목
            </label>
            <input
              id="notice-title"
          ...
          <div>
            <label className="reply-label" htmlFor="notice-body">
              공지 내용
            </label>
            <textarea
              id="notice-body"
...
function SupportPanel({
  ...
  const tabs: Array<{
    ...
    { id: "all", label: "전체", count: inquiries.length },
    {
      id: "pending",
      label: "대기",
      count: inquiries.filter((item) => item.status === "pending").length,
    },
    {
      id: "progress",
      label: "처리중",
      count: inquiries.filter((item) => item.status === "progress").length,
    },
    {
      id: "done",
      label: "완료",
      count: inquiries.filter((item) => item.status === "done").length,
    },
  ];
  const visibleInquiries =
    tab === "all" ? inquiries : inquiries.filter((item) => item.status === tab);
      ...
      <aside className="support-list-panel">
        ...
        <div className="support-list">
          {visibleInquiries.map((item) => (
            <button
              key={item.id}
              className={`support-ticket ${activeId === item.id ? "active" : ""}`}
            ...
            >
              <div className="support-ticket-tags">
                ...
                {item.urgent ? (
                  <span className="urgent-label">긴급</span>
                ) : null}
              </div>
              <b>{item.subject}</b>
      ...
      <section className="support-detail-panel">
        ...
        <div className="support-meta-card">
          ...
          <label>
            <span>담당자 배정</span>
            <select
              defaultValue={activeInquiry.agent || ""}
              onChange={async (event) => {
                await adminApi.updateTicket(activeInquiry.id, {
                  status: "progress",
                  assigneeAdminUserId: event.target.value || null,
                });
                await onReload();
                onToast(
                  event.target.value
                    ? `담당자를 ${event.target.value}으로 배정했어요`
                    : "담당자 배정을 해제했어요",
                );
              }}
            >
          ...
          >
            <b style={{ display: "block", color: "#15803d", marginBottom: 8 }}>
              ...
              {activeInquiry.repliedAt ? (
                <span
                  className="mono"
                  style={{ marginLeft: 8, color: "#78716c", fontWeight: 500 }}
                >
                  {formatHistoryTime(activeInquiry.repliedAt)}
                </span>
              ) : null}
            </b>
        ...
        {!activeInquiry.replyContent ? (
          <>
            <label className="reply-label" htmlFor="support-reply">
              답변 작성
            </label>
            <textarea
              id="support-reply"
            ...
            <div className="reply-actions">
              <label className="faq-check">
                <input
                  type="checkbox"
                  checked={replyFaq}
                  onChange={(event) => setReplyFaq(event.target.checked)}
                />
              ...
              <div className="reply-buttons">
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await adminApi.replyTicket(activeInquiry.id, {
                      body: reply,
                      faq: replyFaq,
                    });
                    await onReload();
...
function BillingPanel({
  ...
  const subscriptionRows = data.billingSubscriptions.filter(
    (item) => item.plan !== "free",
  );
  ...
  return (
    <>
      <div className="billing-kpis">
        <BillingKpi
          label="이번 달 매출"
          value={`₩${(data.billingSummary.monthlyRevenue / 1000000).toFixed(1)}M`}
        />
        <BillingKpi
          label="활성 구독"
          value={data.billingSummary.activeSubscriptions.toLocaleString(
            "ko-KR",
          )}
        />
        <BillingKpi
          label="MRR"
          value={`₩${(data.billingSummary.mrr / 1000000).toFixed(1)}M`}
          accent
        />
        <BillingKpi
          label="결제 실패"
          value={data.billingSummary.failedPaymentCount.toLocaleString("ko-KR")}
          danger
        />
      </div>
      ...
      {tab === "history" ? (
        <BillingHistory
          transactions={data.billingTransactions}
          onReload={onReload}
          onToast={onToast}
        />
      ) : null}
      ...
      {tab === "failed" ? (
        <FailedPayments
          failures={data.failedBilling}
          onReload={onReload}
          onToast={onToast}
        />
      ) : null}
      {tab === "plans" ? (
        <PlanManagement plans={data.planCards} onEdit={openPriceModal} />
      ) : null}
      {priceModal ? (
        <PriceModal
          modal={priceModal}
          ...
          onSave={async () => {
            await adminApi.updatePlanPrice(
              priceModal.key,
              Number(draftPrice || 0),
              timing === "now" ? "IMMEDIATE" : "NEXT_BILLING",
            );
            await onReload();
            onToast(
              `${priceModal.name} 요금제를 ₩${Number(draftPrice || 0).toLocaleString("ko-KR")}로 수정했어요`,
            );
            setPriceModal(null);
          }}
        />
...
function BillingKpi({
  ...
  return (
    <div className="billing-kpi">
      ...
      <b
        className="mono"
        style={{ color: accent ? "#0d9488" : danger ? "#dc2626" : "#111827" }}
      >
        {value}
      </b>
    </div>
...
function BillingHistory({
  ...
  const [refundTarget, setRefundTarget] = useState<BillingTransaction | null>(
    null,
  );
          ...
          <tbody>
            {transactions.map((payment) => (
              <tr key={payment.id}>
                ...
                <td>
                  <Tag meta={planStyle[payment.plan]}>
                    {planLabel[payment.plan] === "무료"
                      ? "Free"
                ...
                <td>
                  <Tag meta={paymentStatus[payment.status]}>
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="price-modal" onClick={(event) => event.stopPropagation()}>
        ...
        <h2>환불 확인</h2>
        <p>
          {payment.user} 사용자의 결제를 환불할까요? 환불 후 결제 상태는 환불로
          유지되고, 사용자는 무료 플랜으로 전환됩니다.
        </p>
        <div
          style={{
            width: "100%",
        ...
        >
          ...
          >
            <span style={{ color: "#64748b" }}>거래 ID</span>
          ...
          >
            <span style={{ color: "#64748b" }}>사용자</span>
            <b>{payment.user}</b>
          </div>
          <div
            style={{
          ...
          >
            <span style={{ color: "#64748b" }}>환불 금액</span>
            <b>{money(payment.amount)}</b>
          </div>
        </div>
        <label
      ...
      {subscriptions.map((item) => (
        <div className="subscription-card" key={item.subscriptionId}>
          ...
          <div style={{ flex: 1 }}>
            <b>{item.user}</b>
            <p>
              시작 {item.started} · 다음 결제{" "}
              <strong>
                {getBillingCycleLabel(item.billingCycle)}{" "}
                {getNextBillingDate(item.started, item.billingCycle)}
              </strong>
            </p>
          </div>
          <div className="subscription-price">
            <Tag meta={planStyle[item.plan]}>
              {planLabel[item.plan] === "무료" ? "Free" : planLabel[item.plan]}
            </Tag>
            <span className="mono">
              {money(item.amount)}
              <small>/월</small>
...
function PlanManagement({
  ...
  plans: PlanCard[];
  onEdit: (plan: { key: Plan; name: string; price: number }) => void;
}) {
  return (
        ...
        {plans.map((plan) => (
          <div className="plan-admin-card" key={plan.key}>
            ...
            >
              ...
              <button
                onClick={() =>
                  onEdit({ key: plan.key, name: plan.name, price: plan.price })
                }
              >
                가격 수정
              </button>
...
function PriceModal({
  ...
  modal: { key: Plan; name: string; price: number };
  value: string;
  timing: "now" | "next";
  onValue: (value: string) => void;
  onTiming: (timing: "now" | "next") => void;
  onClose: () => void;
  onSave: () => void;
}) {
  return (
      ...
      <div className="price-modal">
        ...
        <label className="price-label">
          ...
          <input
            ...
            value={value}
            onChange={(event) =>
              onValue(event.target.value.replace(/[^0-9]/g, ""))
            }
          />
        </label>
        ...
        <div className="timing-grid">
          <button
            className={timing === "now" ? "active" : ""}
            onClick={() => onTiming("now")}
          >
            즉시 적용
          </button>
          <button
            className={timing === "next" ? "active" : ""}
            onClick={() => onTiming("next")}
          >
            다음 결제부터
          </button>
        </div>
